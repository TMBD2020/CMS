...
function DesignationShow() {
    ...
    useEffect(() => {
        document.title = "CMS ::  Designation information";
    }, []);
    ...
    const columns = [
        ...
        { name: 'Designation For', selector: row => row.designationFor, sortable: true },
        ...
        <div> 
             <Link to={`/designation/details/${row.id}`} className="btn text-info btn-outline-light p-2 mb-0 btn-sm">
             <AiOutlineEye size={15} />
             </Link>
        </div> 
        ) },
    ];
    const filteredItems = designation && designation.filter( 
